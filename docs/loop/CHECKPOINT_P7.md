...
|---|---|---|---|---|
| 5 | CORE | W3 | **DEC-05 마이그레이션 규율 위반 발견**(수정 아님, 발견·등재). `migrations/v001_initial_schema.sql` 의 `Watcher_Config.mode` 컬럼(line 67)이 커밋 `02abae24`("WA-CMD-01/02/03 WatcherService")에서 **v001 을 편집해** 추가됨 — 신규 마이그레이션(v008 ALTER)이 아님. v001 은 `CREATE TABLE IF NOT EXISTS` 라 기존 테이블에 컬럼을 못 붙이고, `mode` 를 추가하는 후속 마이그레이션도 없어 **`mode` 추가 이전 생성된 모든 DB(=실 DB, user_version=7)** 에서 watcher 활성화(realtime/idle) 시 `sqlite3.OperationalError: table Watcher_Config has no column named mode` → HTTP 500. CI/단위테스트는 매번 신선한 DB 를 v001 로 만들어 `mode` 를 갖기에 못 잡는 사각지대. W3 실 HTTP 왕복에서 표면화. 규칙 2-3 에 따라 미구현·이슈 등재. | 규칙 2-3: "DEC-01~17 위반을 발견하면 즉시 CORE 로 등재한다". 세 문서(특히 CLAUDE.md §4 DEC-05 "기존 파일 편집 금지, 신규 마이그레이션 추가")에 정면으로 반하는 기존 코드의 위반을 발견한 것이므로 CORE. |
