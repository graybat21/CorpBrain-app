...
- **기술스택 확정 사항: SRS `§3.3 DEC-02`** — FastAPI + Pydantic v2, OpenAPI 3.1이 계약 SSOT

## :white_check_mark: Task Breakdown (실행 계획)
- [ ] CQRS 원칙에 따른 `WorkspaceCreateReq`, `WorkspaceListRes` 등 DTO를 **Pydantic v2 `BaseModel`** 로 정의
- [ ] DTO 필드별 타입, 제약조건(Validation 규칙), 기본값 적용
- [ ] **FastAPI가 생성하는 OpenAPI 3.1 스키마를 SSOT로 확정** 후 export. 프론트엔드 TypeScript 타입은 이 스키마에서 **생성**한다 (수동 작성·이중 관리 금지)
